...
// Comfly API 独立模型列表：gpt-image-2 / gemini-3.1-flash-image-preview / nano-banana-pro。
// 自定义 OpenAI 兼容 API 的模型列表保持不变。
...
function loadSettings() {
  ...
  const savedImageModel = ALL_IMAGE_MODEL_OPTIONS.includes(saved.defaultImageModel)
...
function toComflyReferenceImageValue(image) {
  return isDataImage(image) ? stripDataPrefix(image) : image;
}

function buildComflyImageBody({ model, prompt, ratio, quality, n = 1, images = [] }) {
  const imageSize = qualityToImageSize(quality);
  const aspectRatio = ratioToAspectRatio(ratio) || 'auto';
  const refs = uniqueImages(images).map(toComflyReferenceImageValue).filter(Boolean);
  const body = {
    model,
    prompt,
    n,
...
function isGptImage4kModel(_model) {
  return false;
}
...
async function upscaleImageWithApi({ settings, nodeData, upstreamImages, onRequest }) {
  ...
  const runImagesEditUpscale = async () => {
    ...
    if (isComflyDalleImageModel(requestModel, platform)) {
      const endpoint = buildPlatformEndpoint(platform, '/images/generations');
      onRequest?.(endpoint);
      const sendComflyOnce = async (prompt) => {
        const body = buildComflyImageBody({
          model: requestModel,
          prompt,
          ratio: aspectRatio,
    ...
    try {
      ...
      const message = String(error?.message || '');
      if (/HTTP 422|could not generate an image|image safety|given prompt/i.test(message)) {
        json = await sendOnce(retryPrompt, false);
        ...
        <div className="settings-grid">
          ...
          <div className="settings-form">
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本模型规则：Comfly API 只显示 <code>gpt-image-2</code>、<code>gemini-3.1-flash-image-preview</code>、<code>nano-banana-pro</code> 这 3 个模型；自定义 OpenAI兼容 API 保持原来的 <code>gpt-image-2</code> 和 <code>gemini-3.1-flash-image</code> 不变。分组规则：<code>gpt-image-2</code> 走 <code>default</code>；<code>gemini-3.1-flash-image-preview</code> 走 <code>gemini-t3</code>；<code>nano-banana-pro</code> 走 <code>Gemini优质</code>。Comfly 的 Gemini / Nano 图片模型按文档走 <code>/v1/images/generations</code>，参考图通过 JSON 的 <code>image</code> 数组传入；自定义 OpenAI 兼容 API 下的 Gemini 图片模型如果上游返回 405/404，会自动退回 <code>/v1/chat/completions</code>。高清放大默认使用 <code>nano-banana-pro</code> + <code>4K</code>。
            </div>
          </div>
        </div>
      </div>
