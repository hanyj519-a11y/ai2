...
function GenerateNode({ id, data, selected }) {
  ...
  const fallbackImageModel = getImageModelOptions(platform).includes(settings.defaultImageModel)
    ? settings.defaultImageModel
    : getDefaultImageModelForPlatform(platform);
  const currentImageModel = getImageModelOptions(platform).includes(data.model)
    ? data.model
    : fallbackImageModel;
  ...
  const choosePlatform = (platformId) => {
    ...
    data.updateNode?.(id, {
      platformId,
      model: nextOptions.includes(data.model) ? data.model : getDefaultImageModelForPlatform(nextPlatform),
    });
  };
...
function UpscaleNode({ id, data, selected }) {
  ...
  const settings = data.settings || DEFAULT_SETTINGS;
  const platform = getPlatformById(settings, data.platformId);
  const fallbackUpscaleModel = getDefaultUpscaleModelForPlatform(platform);
  const currentUpscaleModel = getImageModelOptions(platform).includes(data.model)
    ? data.model
    : fallbackUpscaleModel;
  ...
  const choosePlatform = (platformId) => {
    ...
    data.updateNode?.(id, {
      platformId,
      model: nextOptions.includes(data.model) ? data.model : getDefaultUpscaleModelForPlatform(nextPlatform),
    });
  };
  ...
  const runNode = useCallback(async (nodeId) => {
    ...
    try {
      ...
      if (node.type === 'generateNode' || node.type === 'upscaleNode') {
        ...
        const fallbackModel = node.type === 'upscaleNode' ? getDefaultUpscaleModelForPlatform(platform) : settings.defaultImageModel;
