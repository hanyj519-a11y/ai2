...
const APP_PATCH_VERSION = 'v1.5.2-node-model-hardfix';
const DEFAULT_PLATFORM_ID = 'comfly-ai-default-platform';
const CUSTOM_PLATFORM_ID = 'blank-openai-compatible-platform';
const COMFLY_IMAGE_MODEL_OPTIONS = [
  'gpt-image-2',
  'gemini-3.1-flash-image-preview',
  'nano-banana-pro',
];
const CUSTOM_OPENAI_IMAGE_MODEL_OPTIONS = [
  'gpt-image-2',
  'gemini-3.1-flash-image',
];
const IMAGE_MODEL_OPTIONS = COMFLY_IMAGE_MODEL_OPTIONS;
const ALL_IMAGE_MODEL_OPTIONS = Array.from(new Set([
  ...COMFLY_IMAGE_MODEL_OPTIONS,
  ...CUSTOM_OPENAI_IMAGE_MODEL_OPTIONS,
]));
const TEXT_MODEL_OPTIONS = ['gemini-3.1-pro-preview'];
const COMFLY_DEFAULT_GROUP = 'default';
// Comfly API 固定 3 个图片模型；自定义 OpenAI 兼容 API 的模型列表不变。
// gemini-3.1-flash-image-preview 参考文档2走 Generations；nano-banana-pro 参考 pro 文档走 Generations。
// 分组仍按模型自动处理：gpt-image-2 默认分组，gemini preview 走 gemini-t3，nano/pro 文本走 Gemini优质。
const COMFLY_GEMINI_T3_GROUP = 'gemini-t3';
const COMFLY_GEMINI_PREMIUM_GROUP = 'Gemini优质';
const COMFLY_MODEL_GROUP_MAP = {
  'gpt-image-2': COMFLY_DEFAULT_GROUP,
  'gpt-image-2-4k': COMFLY_DEFAULT_GROUP,
  'gemini-3.1-flash-image-preview': COMFLY_GEMINI_T3_GROUP,
  'gemini-3.1-pro-preview': COMFLY_GEMINI_PREMIUM_GROUP,
  'nano-banana-pro': COMFLY_GEMINI_PREMIUM_GROUP,
  'nano-banana-pro-4k': COMFLY_GEMINI_PREMIUM_GROUP,
};
const RATIOS = ['1:1', '2:3', '3:4', '9:16', '16:9', '4:3', '3:2'];
const QUALITIES = ['1k', '2k', '3k', '4k'];
const COUNTS = [1, 2, 3, 4];
const UPSCALE_DEFAULT_MODEL = 'nano-banana-pro';
const UPSCALE_DEFAULT_QUALITY = '4k';
const GPT_IMAGE_SIZE_MAX_EDGE = { '1k': 1024, '2k': 2048, '3k': 3072, '4k': 3840 };
const GPT_IMAGE_MAX_PIXEL_BUDGET = 9437184;
const GEMINI_IMAGE_SIZE_MAX_EDGE = { '1k': 1024, '2k': 2048, '3k': 3072, '4k': 4096 };
const COMFLY_IMAGE_SUPPORTED_RATIOS = ['1:1', '2:3', '3:4', '9:16', '16:9', '4:3', '3:2'];

const DEFAULT_PLATFORMS = [
  {
    id: DEFAULT_PLATFORM_ID,
    name: 'Comfly API（按模型自动分组 / 4K修正版）',
...
function loadSettings() {
  ...
  const activePlatform = platforms.find((item) => item.id === saved.activePlatformId);
  const shouldKeepSavedActive = Boolean(
    activePlatform && (activePlatform.id !== CUSTOM_PLATFORM_ID || activePlatform.baseUrl || activePlatform.apiKey),
  );
  ...
  const normalizedSavedImageModel = saved.defaultImageModel === 'nano-banana-pro-4k'
    ? 'nano-banana-pro'
    : saved.defaultImageModel === 'gemini-3.1-flash-image-preview-4k'
      ? 'gemini-3.1-flash-image-preview'
      : saved.defaultImageModel;
  const savedImageModel = ALL_IMAGE_MODEL_OPTIONS.includes(normalizedSavedImageModel)
    ? normalizedSavedImageModel
    : DEFAULT_SETTINGS.defaultImageModel;
  const nextActivePlatformId = shouldKeepSavedActive ? saved.activePlatformId : DEFAULT_SETTINGS.activePlatformId;
  const nextActivePlatform = platforms.find((item) => item.id === nextActivePlatformId) || platforms[0];
  const platformImageOptions = getImageModelOptions(nextActivePlatform);
  return {
    ...DEFAULT_SETTINGS,
    ...saved,
    platforms,
    activePlatformId: nextActivePlatformId,
    defaultTextModel: savedTextModel,
    defaultImageModel: platformImageOptions.includes(savedImageModel) ? savedImageModel : getDefaultImageModelForPlatform(nextActivePlatform),
  };
}
...
function isComflyPlatform(platformOrBaseUrl) {
  const baseUrl = typeof platformOrBaseUrl === 'object' ? platformOrBaseUrl?.baseUrl : platformOrBaseUrl;
  return /(^|\.)ai\.comfly\.chat/i.test(String(baseUrl || ''));
}

function isCustomOpenAIPlatform(platform) {
  return String(platform?.id || '') === CUSTOM_PLATFORM_ID;
}

function getComflyModelGroup(model, platform) {
  if (!isComflyPlatform(platform)) return '';
  return COMFLY_MODEL_GROUP_MAP[normalizeModelName(model)] || '';
}

function getImageRequestGroup(model, platform) {
  const name = normalizeModelName(model);
  if (isComflyPlatform(platform)) return COMFLY_MODEL_GROUP_MAP[name] || '';
  if (isCustomOpenAIPlatform(platform) && CUSTOM_OPENAI_IMAGE_MODEL_OPTIONS.includes(name)) {
    return COMFLY_DEFAULT_GROUP;
  }
  return '';
}

function withComflyGroupBody(body, requestGroup) {
  if (!requestGroup || requestGroup === COMFLY_DEFAULT_GROUP || !body || typeof body !== 'object' || body instanceof FormData) return body;
  return { ...body, group: requestGroup };
}

function shouldRetryWithoutGroup(error) {
  const message = String(error?.message || error || '').toLowerCase();
  return /unknown parameter|unrecognized|unsupported parameter|extra_forbidden|not permitted|invalid field|未知参数|不支持的参数/.test(message);
}

function canUseHeaderValue(value = '') {
  return /^[\x20-\x7E]+$/.test(String(value || ''));
}

function addComflyGroupHeaders(output, requestGroup = '') {
  // 浏览器 fetch 的 header 只能是 ByteString，中文分组名放 header 会直接报错。
  // 因此中文分组只放在 JSON/FormData 的 group 字段里。
  if (requestGroup && canUseHeaderValue(requestGroup)) {
    output['X-Comfly-Group'] = requestGroup;
    output['X-Model-Group'] = requestGroup;
    output['X-Channel-Group'] = requestGroup;
  }
  return output;
}

function headers(apiKey, requestGroup = '') {
  const output = {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${apiKey || ''}`,
  };
  return addComflyGroupHeaders(output, requestGroup);
}

function formHeaders(apiKey, requestGroup = '') {
  const output = { Authorization: `Bearer ${apiKey || ''}` };
  return addComflyGroupHeaders(output, requestGroup);
}
...
function fitSizeToPixelBudget(width, height, pixelBudget = GPT_IMAGE_MAX_PIXEL_BUDGET) {
  const round16 = (value) => Math.max(256, Math.round(value / 16) * 16);
  let safeWidth = round16(width);
  let safeHeight = round16(height);
  const pixels = safeWidth * safeHeight;
  if (pixels <= pixelBudget) return { width: safeWidth, height: safeHeight };
  const scale = Math.sqrt(pixelBudget / pixels);
  safeWidth = round16(safeWidth * scale);
  safeHeight = round16(safeHeight * scale);
  while ((safeWidth * safeHeight) > pixelBudget && (safeWidth > 256 || safeHeight > 256)) {
    if (safeWidth >= safeHeight && safeWidth > 256) safeWidth = Math.max(256, safeWidth - 16);
    else if (safeHeight > 256) safeHeight = Math.max(256, safeHeight - 16);
    else break;
  }
...
function ratioToOpenAIImageSize(ratio, quality) {
  ...
  const safe = fitSizeToPixelBudget(width, height);
  return `${safe.width}x${safe.height}`;
}
...
function imageRatioNumber(ratio) {
  const [w, h] = String(ratio || '').split(':').map((value) => Number(value));
  return w && h ? w / h : 1;
}

function nearestSupportedImageRatio(width, height) {
  const sourceRatio = (Number(width) || 1) / (Number(height) || 1);
  return COMFLY_IMAGE_SUPPORTED_RATIOS.reduce((best, ratio) => {
    const bestDiff = Math.abs(Math.log(sourceRatio / imageRatioNumber(best)));
    const nextDiff = Math.abs(Math.log(sourceRatio / imageRatioNumber(ratio)));
    return nextDiff < bestDiff ? ratio : best;
  }, '1:1');
}

function targetLongEdgeForQuality(quality, model = '', platform = null) {
  const normalized = String(quality || '1k').toLowerCase();
  const table = isComflyPlatform(platform) && !isGptImageModel(model)
    ? GEMINI_IMAGE_SIZE_MAX_EDGE
    : GPT_IMAGE_SIZE_MAX_EDGE;
  return table[normalized] || table['1k'];
}

function appendComflyNativeImageConfigFields(formData, { ratio, quality }) {
  const imageSize = qualityToImageSize(quality);
  const aspectRatio = ratioToAspectRatio(ratio) || '1:1';
  const imageConfig = { image_size: imageSize, imageSize, aspect_ratio: aspectRatio, aspectRatio };
...
function isComflyDalleImageModel(model, platform) {
  return isComflyPlatform(platform) && !isGptImageModel(model);
}

function buildComflyImageBody({ model, prompt, ratio, quality, images = [] }) {
  const imageSize = qualityToImageSize(quality);
  const aspectRatio = ratioToAspectRatio(ratio) || '1:1';
  const body = {
    model,
  ...
  if (cleanImages.length) body.image = cleanImages;
  return body;
}

function appendComflyImageFormFields(formData, { model, prompt, ratio, quality, n = 1, includeNativeImageConfig = false }) {
  formData.append('model', model);
  formData.append('prompt', prompt);
  formData.append('n', String(n));
  formData.append('response_format', 'b64_json');
  formData.append('aspect_ratio', ratioToAspectRatio(ratio) || 'auto');
  formData.append('image_size', qualityToImageSize(quality));
  if (includeNativeImageConfig) appendComflyNativeImageConfigFields(formData, { ratio, quality });
}

function scaleImageSizeToQuality(width, height, quality = '4k', model = '', platform = null) {
  const originalWidth = Math.max(1, Math.round(Number(width) || 0));
  const originalHeight = Math.max(1, Math.round(Number(height) || 0));
  if (!originalWidth || !originalHeight) return 'auto';
  const target = targetLongEdgeForQuality(quality, model, platform);
  const longest = Math.max(originalWidth, originalHeight);
  const scale = target / longest;
  const round16 = (value) => Math.max(256, Math.round(value / 16) * 16);
  let nextWidth = round16(originalWidth * scale);
  let nextHeight = round16(originalHeight * scale);
  if (isGptImageModel(model)) {
    const safe = fitSizeToPixelBudget(nextWidth, nextHeight);
    nextWidth = safe.width;
...
async function fetchJson(url, body, apiKey, timeoutMs = 600000, requestGroup = '') {
  const requestUrl = buildProxyUrl(url);
  const send = async (payload, includeBodyGroup = true) => {
    const finalBody = includeBodyGroup ? withComflyGroupBody(payload, requestGroup) : payload;
    const res = await apiFetch(url, {
      method: 'POST',
      headers: headers(apiKey, requestGroup),
  ...
  try {
    return await send(body, true);
  } catch (error) {
    if (requestGroup && requestGroup !== COMFLY_DEFAULT_GROUP && shouldRetryWithoutGroup(error)) {
      try {
        return await send(body, false);
      } catch (retryError) {
        throw new Error(`${retryError.message} 实际请求地址：${requestUrl || url}`);
      }
    }
...
async function fetchFormJson(url, formData, apiKey, timeoutMs = 600000, requestGroup = '') {
  const requestUrl = buildProxyUrl(url);
  const send = async (includeBodyGroup = true) => {
    const payload = formData;
    if (includeBodyGroup && requestGroup && requestGroup !== COMFLY_DEFAULT_GROUP && !payload.has('group')) {
      payload.append('group', requestGroup);
    }
    const res = await apiFetch(url, {
      method: 'POST',
      headers: formHeaders(apiKey, requestGroup),
      body: payload,
    }, timeoutMs);
    return await readJsonResponse(res);
  };
...
function resolveNodePlatformId(settings, platformId) {
  const platforms = settings?.platforms?.length ? settings.platforms : DEFAULT_PLATFORMS;
  const exists = platforms.some((item) => item.id === platformId);
  return exists ? platformId : (settings?.activePlatformId || DEFAULT_PLATFORM_ID);
}

function getImageModelOptions(platform, _currentModel = '') {
  // 不同 API 下固定显示不同模型，避免旧缓存里的 Comfly 旧模型重新插入下拉框。
  return isComflyPlatform(platform) ? COMFLY_IMAGE_MODEL_OPTIONS : CUSTOM_OPENAI_IMAGE_MODEL_OPTIONS;
}

function getDefaultImageModelForPlatform(platform) {
  return getImageModelOptions(platform)[0] || DEFAULT_SETTINGS.defaultImageModel;
}

function getDefaultUpscaleModelForPlatform(platform) {
  return isComflyPlatform(platform) ? UPSCALE_DEFAULT_MODEL : getDefaultImageModelForPlatform(platform);
}

function normalizeLegacyImageModelForPlatform(model, platform) {
  const name = String(model || '').trim();
  if (isComflyPlatform(platform)) {
    if (name === 'gemini-3.1-flash-image') return 'gemini-3.1-flash-image-preview';
    if (name === 'gemini-3.1-flash-image-preview-4k') return 'gemini-3.1-flash-image-preview';
    if (name === 'nano-banana-pro-4k') return 'nano-banana-pro';
  }
  return name;
}

function resolveImageNodeModel(settings, nodeData = {}, nodeType = 'generateNode') {
  const platformId = resolveNodePlatformId(settings, nodeData.platformId);
  const platform = getPlatformById(settings, platformId);
  const options = getImageModelOptions(platform);
  const normalized = normalizeLegacyImageModelForPlatform(nodeData.model, platform);
  if (options.includes(normalized)) return normalized;
  if (nodeType === 'upscaleNode') return getDefaultUpscaleModelForPlatform(platform);
  const savedDefault = normalizeLegacyImageModelForPlatform(settings?.defaultImageModel, platform);
  return options.includes(savedDefault) ? savedDefault : getDefaultImageModelForPlatform(platform);
}

function sanitizeImageNodeDataForPlatform(settings, nodeData = {}, nodeType = 'generateNode') {
  const platformId = resolveNodePlatformId(settings, nodeData.platformId);
  const platform = getPlatformById(settings, platformId);
  const model = resolveImageNodeModel(settings, { ...nodeData, platformId }, nodeType);
  const patch = { platformId, model };
  if (nodeType === 'upscaleNode') patch.quality = nodeData.quality || UPSCALE_DEFAULT_QUALITY;
  return patch;
}

function sameNodePatch(nodeData = {}, patch = {}) {
  return Object.keys(patch).every((key) => nodeData[key] === patch[key]);
}

function getRequestGroupDisplay(model, platform) {
  if (!isComflyPlatform(platform)) return 'default';
  return getImageRequestGroup(model, platform) || COMFLY_DEFAULT_GROUP;
}
...
function groupRunnableNodesByDependencyLevel(nodes = [], edges = []) {
  const runnableTypes = new Set(['reverseNode', 'generateNode', 'upscaleNode']);
  const runnable = nodes.filter((node) => runnableTypes.has(node.type));
  const runnableIds = new Set(runnable.map((node) => node.id));
  const remaining = new Set(runnableIds);
  const levels = [];

  while (remaining.size) {
    const ready = runnable.filter((node) => {
      if (!remaining.has(node.id)) return false;
      return !edges.some((edge) => edge.target === node.id && remaining.has(edge.source) && runnableIds.has(edge.source));
    });

    const level = ready.length ? ready : runnable.filter((node) => remaining.has(node.id)).slice(0, 1);
...
function waitForCanvasStateFlush() {
...
function supportsImagesApiGeneration(model) {
  ...
  return (
    ...
    /gemini.*image/.test(name) ||
    /nano.*banana/.test(name)
  );
}
...
function shouldFallbackToChatImageRoute(error, model, platform) {
  if (!isCustomOpenAIPlatform(platform)) return false;
  if (isGptImageModel(model)) return false;
  if (!supportsChatImageGeneration(model)) return false;
  const message = String(error?.message || error || '');
  return /HTTP 404|HTTP 405|method not allowed|not found|not supported|unsupported endpoint|接口不存在|不支持/i.test(message);
}

function buildChatImageContent(prompt, images = []) {
...
async function requestChatImageRoute({ platform, model, prompt, images = [], requestGroup = '', onRequest }) {
  ...
  const body = {
    ...
    messages: [
      {
        role: 'user',
        content: buildChatImageContent(prompt, images),
      },
    ],
  };
  onRequest?.(endpoint);
  const json = await fetchJson(endpoint, body, platform.apiKey, 600000, requestGroup);
  return extractFirstReturnedImage(json, images);
}
...
async function imageSourceToDataUrl(source) {
  ...
  if (isHttpUrl(source)) {
    const response = await apiFetch(buildProxyUrl(source), { method: 'GET' }, 120000);
    if (!response.ok) throw new Error(`输出图下载失败：HTTP ${response.status}`);
    return await blobToDataUrl(await response.blob());
  }
  return normalizeImageSource(source);
}

async function resizeImageToTargetLongEdge(source, targetLongEdge) {
  ...
  if (!longest || longest >= targetLongEdge * 0.98) return source;
...
async function ensureImageMeetsTargetLongEdge(source, quality, model, platform) {
  const targetLongEdge = targetLongEdgeForQuality(quality, model, platform);
  const dimensions = await getImageDimensions(source);
  const longest = dimensions ? Math.max(dimensions.width, dimensions.height) : 0;
  if (longest >= targetLongEdge * 0.98) return source;
  return await resizeImageToTargetLongEdge(source, targetLongEdge);
}
...
async function generateImagesWithApi({ settings, nodeData, upstreamText, upstreamImages, onRequest }) {
  ...
  const platformImageOptions = getImageModelOptions(platform);
  const fallbackSelectedModel = platformImageOptions.includes(settings.defaultImageModel)
    ? settings.defaultImageModel
    : getDefaultImageModelForPlatform(platform);
  const rawSelectedModel = nodeData.model || fallbackSelectedModel;
  const selectedModel = platformImageOptions.includes(rawSelectedModel) && (supportsImagesApiGeneration(rawSelectedModel) || supportsChatImageGeneration(rawSelectedModel))
    ? rawSelectedModel
    : fallbackSelectedModel;
  const requestGroup = getImageRequestGroup(selectedModel, platform);
  ...
  const runSingleGeneration = async () => {
    ...
    const runImagesGeneration = async (requestModel) => {
      ...
      const body = isComflyDalleImageModel(apiModel, platform)
        ? buildComflyImageBody({ model: apiModel, prompt: singlePrompt, ratio, quality, images: [] })
        : {
            model: apiModel,
            prompt: singlePrompt,
            n: 1,
      ...
      try {
        ...
        if (shouldFallbackToChatImageRoute(error, apiModel, platform)) {
          return await requestChatImageRoute({
            platform,
            model: apiModel,
            prompt: singlePrompt,
            images: [],
    ...
    const runImagesEdit = async (requestModel) => {
      ...
      if (isComflyDalleImageModel(apiModel, platform)) {
        appendComflyImageFormFields(formData, { model: apiModel, prompt: singlePrompt, ratio, quality, n: 1 });
      } else {
      ...
      onRequest?.(endpoint);
      try {
        const json = await fetchFormJson(endpoint, formData, platform.apiKey, 600000, requestGroup);
    ...
    if (supportsImagesApiGeneration(selectedModel)) {
      if (apiImageInputs.length && isComflyDalleImageModel(normalizeImageRequestModel(selectedModel), platform)) {
        const apiModel = normalizeImageRequestModel(selectedModel);
        const body = buildComflyImageBody({
          model: apiModel,
          prompt: singlePrompt,
          ratio,
          quality,
...
async function upscaleImageWithApi({ settings, nodeData, upstreamImages, onRequest }) {
  ...
  const platformImageOptions = getImageModelOptions(platform);
  const fallbackSelectedModel = getDefaultUpscaleModelForPlatform(platform);
  const rawSelectedModel = nodeData.model || fallbackSelectedModel;
  const selectedModel = platformImageOptions.includes(rawSelectedModel) && (supportsImagesApiGeneration(rawSelectedModel) || supportsChatImageGeneration(rawSelectedModel))
    ? rawSelectedModel
    : fallbackSelectedModel;
  ...
  const redrawQuality = nodeData.quality || UPSCALE_DEFAULT_QUALITY;
  ...
  const aspectRatio = dimensions ? nearestSupportedImageRatio(dimensions.width, dimensions.height) : '1:1';
  const targetLongEdge = targetLongEdgeForQuality(redrawQuality, requestModel, platform);
  const targetSize = dimensions ? scaleImageSizeToQuality(dimensions.width, dimensions.height, redrawQuality, requestModel, platform) : `${targetLongEdge}x${targetLongEdge}`;
  const dimensionText = dimensions
    ? `Original image size: ${dimensions.width}x${dimensions.height}. Recreate it at ${qualityToImageSize(redrawQuality)} resolution with the same aspect ratio. Target long edge is about ${targetLongEdge}px and target size is about ${targetSize}.`
    : `Recreate this image at ${qualityToImageSize(redrawQuality)} resolution with the same aspect ratio. Target long edge is about ${targetLongEdge}px.`;
  const redrawPrompt = `Recreate this reference image as a high-resolution ${qualityToImageSize(redrawQuality)} image. Keep the same composition, subject, colors, proportions, perspective, structure, and important details. Do not redesign, do not deform, do not add extra elements, and do not change the layout. This is a faithful high-resolution redraw of the original image, not a new design. ${dimensionText} Return one single final image.`;
  const retryPrompt = `Recreate this image in ${qualityToImageSize(redrawQuality)}. Keep the same composition, subject, proportions, colors, and details. Do not change or redesign anything. Return one image.`;

  const buildUpscaleFormData = async (prompt, includeNativeImageConfig = false) => {
    const formData = new FormData();
    if (isComflyDalleImageModel(requestModel, platform)) {
      appendComflyImageFormFields(formData, {
        model: requestModel,
        prompt,
        ratio: aspectRatio,
        quality: redrawQuality,
        n: 1,
        includeNativeImageConfig,
      });
    } else {
      ...
      formData.append('quality', qualityToOpenAIQuality(redrawQuality));
    }
  ...
  const runImagesGenerationUpscale = async () => {
    const endpoint = buildPlatformEndpoint(platform, '/images/generations');
    onRequest?.(endpoint);
    const sendOnce = async (prompt) => {
      const body = buildComflyImageBody({
        model: requestModel,
        prompt,
        ratio: aspectRatio,
      ...
      return await fetchJson(endpoint, body, platform.apiKey, 600000, requestGroup);
    };
    let json;
    try {
      json = await sendOnce(redrawPrompt);
    } catch (error) {
      const message = String(error?.message || '');
      if (/HTTP 422|could not generate an image|image safety|given prompt/i.test(message)) {
        json = await sendOnce(retryPrompt);
      } else if (shouldFallbackToChatImageRoute(error, requestModel, platform)) {
        const output = await requestChatImageRoute({
          platform,
          model: requestModel,
          prompt: redrawPrompt,
          images: [apiImage],
        ...
        return await ensureImageMeetsTargetLongEdge(output, redrawQuality, requestModel, platform);
      } else {
        throw error;
      }
    }
    const output = extractFirstReturnedImage(json, [firstImage]);
    return await ensureImageMeetsTargetLongEdge(output, redrawQuality, requestModel, platform);
  };

  const runImagesEditUpscale = async () => {
    if (isComflyDalleImageModel(requestModel, platform)) return await runImagesGenerationUpscale();
    const endpoint = buildPlatformEndpoint(platform, '/images/edits');
    onRequest?.(endpoint);
    const sendOnce = async (prompt, includeNativeImageConfig) => {
      const formData = await buildUpscaleFormData(prompt, includeNativeImageConfig);
      return await fetchFormJson(endpoint, formData, platform.apiKey, 600000, requestGroup);
    };
    let json;
    try {
      json = await sendOnce(redrawPrompt, isComflyDalleImageModel(requestModel, platform));
    } catch (error) {
      const message = String(error?.message || '');
      if (/HTTP 422|could not generate an image|image safety|given prompt/i.test(message)) {
        json = await sendOnce(retryPrompt, isComflyDalleImageModel(requestModel, platform));
      } else if (shouldRetryWithoutGroup(error)) {
        json = await sendOnce(redrawPrompt, false);
      } else if (shouldFallbackToChatImageRoute(error, requestModel, platform)) {
        const output = await requestChatImageRoute({
          platform,
          model: requestModel,
  ...
  throw new Error(`当前模型 ${selectedModel} 没有可用的高清重绘路由。`);
}
...
async function reversePromptWithApi({ settings, nodeData, upstreamImages }) {
  ...
  const requestGroup = getComflyModelGroup(model, platform);
  const body = {
    model,
    messages: [
      {
  ...
  const json = await fetchJson(buildPlatformEndpoint(platform, '/chat/completions'), body, platform.apiKey, 600000, requestGroup);
  const text =
...
function GenerateNode({ id, data, selected }) {
  ...
  const resolvedPlatformId = resolveNodePlatformId(settings, data.platformId);
  const platform = getPlatformById(settings, resolvedPlatformId);
  const currentImageModel = resolveImageNodeModel(settings, { ...data, platformId: resolvedPlatformId }, 'generateNode');
  const modelOptions = getImageModelOptions(platform, currentImageModel);
  const choosePlatform = (platformId) => {
    const nextPlatform = getPlatformById(settings, platformId);
    const nextModel = resolveImageNodeModel(settings, { ...data, platformId }, 'generateNode');
    data.updateNode?.(id, {
      platformId,
      model: getImageModelOptions(nextPlatform).includes(nextModel) ? nextModel : getDefaultImageModelForPlatform(nextPlatform),
    });
  };
  ...
  return (
    <NodeShell title={data.title || '图片生成'} icon={Sparkles} status={data.status} selected={selected} className="generate-node">
      ...
      <div className="node-option-wrap nodrag nopan">
        <NodeApiSelect settings={settings} value={resolvedPlatformId} onChange={choosePlatform} />
        <div className="generate-model-row">
          <IconBadge
            icon={Wand2}
            className="model-badge full"
            text={currentImageModel || '选择模型'}
            title="模型"
            active={openMenu === 'model'}
            onClick={() => toggleMenu('model')}
          >
            <ModelLabel value={currentImageModel || '选择模型'} />
          </IconBadge>
        </div>
...
function UpscaleNode({ id, data, selected }) {
  ...
  const resolvedPlatformId = resolveNodePlatformId(settings, data.platformId);
  const platform = getPlatformById(settings, resolvedPlatformId);
  const currentUpscaleModel = resolveImageNodeModel(settings, { ...data, platformId: resolvedPlatformId }, 'upscaleNode');
  const modelOptions = getImageModelOptions(platform, currentUpscaleModel);
  const choosePlatform = (platformId) => {
    const nextPlatform = getPlatformById(settings, platformId);
    const nextModel = resolveImageNodeModel(settings, { ...data, platformId }, 'upscaleNode');
    data.updateNode?.(id, {
      platformId,
      model: getImageModelOptions(nextPlatform).includes(nextModel) ? nextModel : getDefaultUpscaleModelForPlatform(nextPlatform),
      quality: data.quality || UPSCALE_DEFAULT_QUALITY,
    });
  };
  ...
  return (
    <NodeShell title={data.title || '高清放大'} icon={Sparkles} status={data.status} selected={selected} className="upscale-node">
      ...
      <div className="preview-row">
        ...
        {!(data.upstreamImages || []).length ? <span className="empty-preview">连接图片后可一键输出大图，默认 4K 重绘</span> : null}
      </div>
      <div className="upscale-auto-note">
      ...
      <div className="node-option-wrap reverse-option-wrap nodrag nopan">
        ...
        <div className="generate-model-row">
          <IconBadge
            icon={Wand2}
            className="model-badge full"
            text={currentUpscaleModel}
            title="高清放大模型"
            active={openMenu === 'model'}
            onClick={() => setOpenMenu((current) => current === 'model' ? null : 'model')}
          >
            <ModelLabel value={currentUpscaleModel} />
          </IconBadge>
        </div>
        {openMenu === 'model' ? (
          <ChoicePopover
          ...
          <div className="platform-list">
            ...
            {settings.platforms.map((item) => (
              <button
                ...
                type="button"
                onClick={() => {
                  const nextPlatform = getPlatformById(settings, item.id);
                  const nextOptions = getImageModelOptions(nextPlatform);
                  setSettings({
                    ...settings,
                    activePlatformId: item.id,
                    defaultImageModel: nextOptions.includes(settings.defaultImageModel) ? settings.defaultImageModel : getDefaultImageModelForPlatform(nextPlatform),
                  });
                }}
              >
          ...
          <div className="settings-form">
            ...
            <div className="two-cols">
              ...
              <label>
                ...
                >
                  {getImageModelOptions(active, settings.defaultImageModel).map((model) => <option key={model}>{model}</option>)}
                </select>
              </label>
            </div>
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本分组规则：Comfly API 固定 3 个图片模型：<code>gpt-image-2</code>、<code>gemini-3.1-flash-image-preview</code>、<code>nano-banana-pro</code>；<code>gpt-image-2</code> 走 <code>default</code>，<code>gemini-3.1-flash-image-preview</code> 走 <code>gemini-t3</code>，<code>nano-banana-pro</code> 和 <code>gemini-3.1-pro-preview</code> 走 <code>Gemini优质</code>。自定义 OpenAI兼容 API 的图片生成/高清放大模型保持不变：<code>gpt-image-2</code> 和 <code>gemini-3.1-flash-image</code>；这两个模型都按 <code>default</code> 默认分组发送，不走 gemini / Gemini优质 分组。路由规则：Comfly 的 <code>gemini-3.1-flash-image-preview</code> 和 <code>nano-banana-pro</code> 按文档走 <code>/v1/images/generations</code>，参考图通过 JSON 的 <code>image</code> 数组传入；<code>gpt-image-2</code> 无参考图走 <code>/v1/images/generations</code>，有参考图或高清放大走 <code>/v1/images/edits</code>。反推提示词走 <code>/v1/chat/completions</code>。高清放大默认使用 <code>nano-banana-pro</code> + <code>4K</code>。多张图片会并行发送请求。
            </div>
          </div>
        </div>
      </div>
...
function Workspace() {
  ...
  const addNode = useCallback((type, position, extra = {}) => {
    ...
    const defaults = {
      ...
      generateNode: {
        ...
        model: getImageModelOptions(getActivePlatform(settings)).includes(settings.defaultImageModel) ? settings.defaultImageModel : getDefaultImageModelForPlatform(getActivePlatform(settings)),
        ratio: '1:1',
        quality: '1k',
        count: 1,
        upstreamImages: [],
      ...
      upscaleNode: {
        title: '高清放大 / 大图重绘',
        platformId: settings.activePlatformId,
        model: getDefaultUpscaleModelForPlatform(getActivePlatform(settings)),
        quality: UPSCALE_DEFAULT_QUALITY,
        upstreamImages: [],
        status: '等待运行',
      },
      reverseNode: {
  ...
  useEffect(() => {
    setNodes((prevNodes) => {
      let changed = false;
      const nextNodes = prevNodes.map((node) => {
        if (!['generateNode', 'upscaleNode'].includes(node.type)) return node;
        const patch = sanitizeImageNodeDataForPlatform(settings, node.data, node.type);
        if (sameNodePatch(node.data, patch)) return node;
        changed = true;
        return { ...node, data: { ...node.data, ...patch } };
      });
      return changed ? nextNodes : prevNodes;
    });
  }, [settings.activePlatformId, settings.defaultImageModel, settings.platforms, setNodes]);
  ...
  const runNode = useCallback(async (nodeId) => {
    ...
    try {
      ...
      let runnableData = node.data;
      if (node.type === 'generateNode' || node.type === 'upscaleNode') {
        const fixedPatch = sanitizeImageNodeDataForPlatform(settings, node.data, node.type);
        runnableData = { ...node.data, ...fixedPatch };
        if (!sameNodePatch(node.data, fixedPatch)) updateNode(nodeId, fixedPatch);
        const platform = getPlatformById(settings, runnableData.platformId);
        addLog(`使用 API：${getPlatformLabel(platform)}；模型：${runnableData.model}；分组：${getRequestGroupDisplay(runnableData.model, platform)}；上游参考图：${upstream.images.length} 张。`, 'info');
      }
      if (node.type === 'generateNode') {
        let actualRequestCount = 0;
        const images = await generateImagesWithApi({
          settings,
          nodeData: runnableData,
      ...
      if (node.type === 'upscaleNode') {
        ...
        createResultImageNodes(node, images);
        addLog(`高清放大 / 大图重绘完成：得到 ${images.length} 张图片；前端实际发送 ${actualRequestCount} 次请求。`, actualRequestCount === 1 ? 'success' : 'warn');
      }
  ...
  const runNodeLevels = useCallback(async (runnableNodes, allEdges, emptyMessage) => {
    if (!runnableNodes.length) {
      addLog(emptyMessage, 'info');
      return;
    }

    const levels = groupRunnableNodesByDependencyLevel(runnableNodes, allEdges);
    for (const level of levels) {
      addLog(level.length > 1 ? `并行运行 ${level.length} 个互不依赖的任务。` : `运行任务：${level[0]?.data?.title || level[0]?.type || '节点'}。`, 'info');
      // 同一层互不依赖，可以并行；下一层会等上游结果写回画布后再运行。
      await Promise.all(level.map((node) => runNodeRef.current?.(node.id)));
  ...
  const runAll = useCallback(async () => {
    ...
    try {
      ...
      const runnable = allNodes.filter((node) => ['reverseNode', 'generateNode', 'upscaleNode'].includes(node.type));
      await runNodeLevels(runnable, allEdges, '画布里还没有可运行的节点。');
    } finally {
      setQueueRunning(false);
    }
  }, [queueRunning, reactFlow, runNodeLevels]);

  const runGroup = useCallback(async (groupId) => {
    const allNodes = reactFlow.getNodes();
    const allEdges = reactFlow.getEdges();
    const groupChildren = allNodes.filter((node) => node.parentId === groupId && ['reverseNode', 'generateNode', 'upscaleNode'].includes(node.type));
    await runNodeLevels(groupChildren, allEdges, '这个分组里没有可运行的图片生成、高清放大或反推节点。');
  }, [reactFlow, runNodeLevels]);
    ...
    <div className="app-shell">
      ...
      <main className="canvas-area" ref={flowWrapperRef} onDoubleClickCapture={onCanvasDoubleClickCapture}>
        ...
        <div className="bottom-status">
          ...
          <span>版本：{APP_PATCH_VERSION}</span>
          <span>V1.4.2</span>
        </div>
      </main>
