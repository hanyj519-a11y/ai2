...
// Comfly 图片模型下拉框只保留 3 个：gpt-image-2 / gemini-3.1-flash-image-preview / nano-banana-pro。
// 旧缓存里的 *-4k 别名会在请求前自动归一到正式模型名，不再显示在下拉框里。
...
const COMFLY_MODEL_GROUP_MAP = {
  ...
  'gemini-3.1-flash-image-preview-4k': COMFLY_GEMINI_T3_GROUP,
...
function buildComflyImageBody({ model, prompt, ratio, quality, n = 1, images = [] }) {
  const imageSize = qualityToImageSize(quality);
  const aspectRatio = ratioToAspectRatio(ratio) || 'auto';
  const body = {
    model,
    prompt,
    n,
  ...
  if (imageList.length) body.image = imageList;
  return body;
}
  ...
  const runSingleGeneration = async () => {
    ...
    const runImagesGeneration = async (requestModel) => {
      ...
      const body = isComflyDalleImageModel(apiModel, platform)
        ? buildComflyImageBody({ model: apiModel, prompt: singlePrompt, ratio, quality, n: 1, images: apiImageInputs })
        : {
            model: apiModel,
            prompt: singlePrompt,
            n: 1,
    ...
    if (supportsImagesApiGeneration(selectedModel)) {
      const normalizedSelectedModel = normalizeImageRequestModel(selectedModel);
      if (apiImageInputs.length && isComflyDalleImageModel(normalizedSelectedModel, platform)) {
...
async function upscaleImageWithApi({ settings, nodeData, upstreamImages, onRequest }) {
  ...
  const runImagesEditUpscale = async () => {
    const useComflyGenerations = isComflyDalleImageModel(requestModel, platform);
    const endpoint = buildPlatformEndpoint(platform, useComflyGenerations ? '/images/generations' : '/images/edits');
    onRequest?.(endpoint);
    const sendOnce = async (prompt, includeNativeImageConfig) => {
      if (useComflyGenerations) {
        ...
        <div className="settings-grid">
          ...
          <div className="settings-form">
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本模型规则：Comfly API 只显示 3 个图片模型：<code>gpt-image-2</code>、<code>gemini-3.1-flash-image-preview</code>、<code>nano-banana-pro</code>；自定义 OpenAI兼容 API 的图片生成/高清放大模型保持不变：<code>gpt-image-2</code>、<code>gemini-3.1-flash-image</code>。分组规则：<code>gpt-image-2</code> 走 <code>default</code>；<code>gemini-3.1-flash-image-preview</code> 走 <code>gemini-t3</code>；<code>nano-banana-pro</code> 和 <code>gemini-3.1-pro-preview</code> 走 <code>Gemini优质</code>。路由规则：Comfly 的 <code>gemini-3.1-flash-image-preview</code> / <code>nano-banana-pro</code> 按文档走 <code>/v1/images/generations</code>，参考图放入 <code>image</code> 数组；<code>gpt-image-2</code> 有参考图或高清放大仍走 <code>/v1/images/edits</code>。高清放大默认使用 <code>nano-banana-pro</code> + <code>4K</code>。多张图片会并行发送请求。
            </div>
          </div>
        </div>
      </div>
