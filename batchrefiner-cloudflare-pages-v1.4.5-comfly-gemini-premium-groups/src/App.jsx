...
const COMFLY_GEMINI_GROUP = 'gemini优质';
const COMFLY_MODEL_GROUP_MAP = {
  'gpt-image-2': COMFLY_DEFAULT_GROUP,
  'gpt-image-2-4k': COMFLY_DEFAULT_GROUP,
  'gemini-3.1-flash-image-preview': COMFLY_GEMINI_GROUP,
  'gemini-3.1-pro-preview': COMFLY_GEMINI_GROUP,
  'nano-banana-pro-4k': COMFLY_GEMINI_GROUP,
};
...
const DEFAULT_PLATFORMS = [
  {
    ...
    name: 'Comfly API（gpt-image-2=default，Gemini/Nano=Gemini优质）',
...
async function upscaleImageWithApi({ settings, nodeData, upstreamImages, onRequest }) {
  ...
  const prompt = `请把这张图片做高清细节增强：提升清晰度、边缘锐利度、材质纹理和商业摄影质感，降低噪点与压缩痕迹。
  ...
  return (
    <NodeShell title={data.title || '高清放大'} icon={Sparkles} status={data.status} selected={selected} className="upscale-node">
      ...
      <div className="upscale-auto-note">
        不需要填写提示词，连接图片后点击运行，会自动高清增强并尽量保持原图尺寸、构图和主体不变。
      </div>
        ...
        <div className="settings-grid">
          ...
          <div className="settings-form">
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本分组规则：<code>gpt-image-2</code> 走 <code>default</code> 分组；<code>gemini-3.1-flash-image-preview</code>、<code>gemini-3.1-pro-preview</code>、<code>nano-banana-pro-4k</code> 会按 <code>gemini优质 / Gemini优质</code> 分组发送。路由规则：<code>gpt-image-2</code> 无参考图走 <code>/v1/images/generations</code>，有参考图或高清放大走 <code>/v1/images/edits</code>；Gemini/Nano Banana 走 <code>/v1/chat/completions</code>。多张图片会并行发送请求。
            </div>
          </div>
        </div>
      </div>
