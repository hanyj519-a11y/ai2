...
const UPSCALE_DEFAULT_MODEL = 'gpt-image-2';
const UPSCALE_DEFAULT_QUALITY = '4k';
const GPT_IMAGE_SIZE_MAX_EDGE = { '1k': 1024, '2k': 2048, '3k': 3072, '4k': 3840 };
const GPT_IMAGE_MAX_LONG_EDGE = 3840;
const GPT_IMAGE_MAX_PIXEL_BUDGET = 3840 * 2160;
const GPT_IMAGE_EXPERIMENTAL_PIXEL_BUDGET = 2560 * 1440;
...
function loadSettings() {
  ...
  const savedImageModel = IMAGE_MODEL_OPTIONS.includes(normalizedSavedImageModel)
  ...
  return {
    ...
    ...saved,
    platforms,
    activePlatformId: shouldKeepSavedActive ? saved.activePlatformId : DEFAULT_SETTINGS.activePlatformId,
...
function floorToImageStep(value, min = 256) {
  return Math.max(min, Math.floor((Number(value) || min) / IMAGE_SIZE_STEP) * IMAGE_SIZE_STEP);
}

function ceilToImageStep(value, min = 256) {
  return Math.max(min, Math.ceil((Number(value) || min) / IMAGE_SIZE_STEP) * IMAGE_SIZE_STEP);
}

function normalizeGptImageDimensions(width, height, options = {}) {
  const maxLongEdge = options.maxLongEdge || GPT_IMAGE_MAX_LONG_EDGE;
  const maxPixels = options.maxPixels || GPT_IMAGE_MAX_PIXEL_BUDGET;
  const maxAspect = options.maxAspect || GPT_IMAGE_MAX_ASPECT_RATIO;
  let originalWidth = Math.max(1, Number(width) || 0);
  let originalHeight = Math.max(1, Number(height) || 0);
  if (!originalWidth || !originalHeight) return { width: 1024, height: 1024, experimental: false };

  const sourceRatio = originalWidth / originalHeight;
  const clampedRatio = Math.min(maxAspect, Math.max(1 / maxAspect, sourceRatio));
  const requestedLongEdge = Math.min(Math.max(originalWidth, originalHeight), maxLongEdge);
  let nextWidth;
  let nextHeight;

  if (sourceRatio >= 1) {
    nextWidth = requestedLongEdge;
    nextHeight = nextWidth / clampedRatio;
  } else {
    nextHeight = requestedLongEdge;
    nextWidth = nextHeight * clampedRatio;
  }

  if (nextWidth * nextHeight > maxPixels) {
    const areaScale = Math.sqrt(maxPixels / (nextWidth * nextHeight));
    nextWidth *= areaScale;
    nextHeight *= areaScale;
  }

  nextWidth = floorToImageStep(nextWidth);
  nextHeight = floorToImageStep(nextHeight);

  const enforceRatio = () => {
    if (nextWidth / nextHeight > maxAspect) nextHeight = ceilToImageStep(nextWidth / maxAspect);
    if (nextHeight / nextWidth > maxAspect) nextWidth = ceilToImageStep(nextHeight / maxAspect);
  };
  enforceRatio();

  while ((nextWidth > maxLongEdge || nextHeight > maxLongEdge || nextWidth * nextHeight > maxPixels) && (nextWidth > 256 || nextHeight > 256)) {
    if (nextWidth >= nextHeight && nextWidth > 256) nextWidth = Math.max(256, nextWidth - IMAGE_SIZE_STEP);
    else if (nextHeight > 256) nextHeight = Math.max(256, nextHeight - IMAGE_SIZE_STEP);
    enforceRatio();
  }
  ...
  return {
    width: nextWidth,
    height: nextHeight,
    experimental: nextWidth * nextHeight > GPT_IMAGE_EXPERIMENTAL_PIXEL_BUDGET,
  };
}

function fitSizeToPixelBudget(width, height, pixelBudget = GPT_IMAGE_MAX_PIXEL_BUDGET) {
  return normalizeGptImageDimensions(width, height, { maxPixels: pixelBudget });
}
...
function describeImageSize(size) {
  return `${size.width}×${size.height}${size.experimental ? '（实验性 3K/4K）' : ''}`;
}
...
function ratioToOpenAIImageSize(ratio, quality) {
  ...
  return formatImageSize(normalizeGptImageDimensions(width, height));
}
...
function clampImageSizeToMaxEdge(width, height, maxEdge = GPT_IMAGE_MAX_LONG_EDGE) {
  const normalized = normalizeGptImageDimensions(width, height, { maxLongEdge: maxEdge });
  return formatImageSize(normalized);
}
...
function scaleImageSizeToQuality(width, height, quality = '4k', model = '', platform = null) {
  ...
  const rawWidth = originalWidth * scale;
  const rawHeight = originalHeight * scale;
  if (isGptImageModel(model)) {
    return formatImageSize(normalizeGptImageDimensions(rawWidth, rawHeight));
  }
  return `${floorToImageStep(rawWidth)}x${floorToImageStep(rawHeight)}`;
}
...
async function ensureImageMeetsTargetLongEdge(source, quality, model, platform, requestedSize = '') {
  const requestedParts = String(requestedSize || '').split('x').map((value) => Number(value));
  const sizeLongEdge = requestedParts.length === 2 && requestedParts.every(Boolean)
    ? Math.max(requestedParts[0], requestedParts[1])
    : 0;
  const targetLongEdge = sizeLongEdge || targetLongEdgeForQuality(quality, model, platform);
...
async function generateImagesWithApi({ settings, nodeData, upstreamText, upstreamImages, onRequest }) {
  ...
  const rawSelectedModel = nodeData.model || settings.defaultImageModel;
  const selectedModel = supportsImagesApiGeneration(rawSelectedModel) || supportsChatImageGeneration(rawSelectedModel)
    ? rawSelectedModel
    : settings.defaultImageModel;
  const requestGroup = getComflyModelGroup(selectedModel, platform);
...
async function upscaleImageWithApi({ settings, nodeData, upstreamImages, onRequest }) {
  ...
  const rawSelectedModel = nodeData.model || UPSCALE_DEFAULT_MODEL;
  const selectedModel = supportsImagesApiGeneration(rawSelectedModel) || supportsChatImageGeneration(rawSelectedModel)
    ? rawSelectedModel
  ...
  const fallbackTargetSize = isGptImageModel(requestModel)
    ? formatImageSize(normalizeGptImageDimensions(targetLongEdge, targetLongEdge))
    : `${targetLongEdge}x${targetLongEdge}`;
  const targetSize = dimensions ? scaleImageSizeToQuality(dimensions.width, dimensions.height, redrawQuality, requestModel, platform) : fallbackTargetSize;
  const [targetWidth, targetHeight] = String(targetSize).split('x').map((value) => Number(value));
  const normalizedTarget = isGptImageModel(requestModel) && targetWidth && targetHeight
    ? normalizeGptImageDimensions(targetWidth, targetHeight)
    : null;
  const targetSizeLabel = normalizedTarget ? describeImageSize(normalizedTarget) : targetSize;
  const dimensionText = dimensions
    ? `Original image size: ${dimensions.width}x${dimensions.height}. Recreate it at ${qualityToImageSize(redrawQuality)} resolution with the same aspect ratio. Target long edge is about ${targetLongEdge}px and safe request size is ${targetSizeLabel}.`
    : `Recreate this image at ${qualityToImageSize(redrawQuality)} resolution with the same aspect ratio. Target long edge is about ${targetLongEdge}px and safe request size is ${targetSizeLabel}.`;
  ...
  const runImagesEditUpscale = async () => {
    ...
    return await ensureImageMeetsTargetLongEdge(output, redrawQuality, requestModel, platform, targetSize);
  };
  ...
  return (
    <NodeShell title={data.title || '高清放大'} icon={Sparkles} status={data.status} selected={selected} className="upscale-node">
      ...
      <div className="upscale-auto-note">
        不需要填写提示词。可直接选择 API、模型和 1K/2K/3K/4K 尺寸；gpt-image-2 会自动压到 3840×2160 像素预算、单边 3840px、最大 3:1、16px 整数倍的安全尺寸。
      </div>
      <div className="node-option-wrap reverse-option-wrap nodrag nopan">
        ...
        <div className="generate-control-row upscale-control-row">
          <IconBadge
            icon={Eye}
            text={data.quality || UPSCALE_DEFAULT_QUALITY}
            title="高清尺寸"
        ...
        {openMenu === 'quality' ? (
          <ChoicePopover
            type="quality"
            options={QUALITIES}
            value={data.quality || UPSCALE_DEFAULT_QUALITY}
            onPick={(quality) => data.updateNode?.(id, { quality })}
            onClose={() => setOpenMenu(null)}
          />
        ) : null}
        ...
        <div className="settings-grid">
          ...
          <div className="settings-form">
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本分组规则：<code>gpt-image-2</code> 走 <code>default</code>；<code>gemini-3.1-flash-image-preview</code> 走 <code>gemini-t3</code>；<code>nano-banana-pro</code> 和 <code>gemini-3.1-pro-preview</code> 走 <code>Gemini优质</code>。路由规则：图片生成无参考图走 <code>/v1/images/generations</code>，有参考图或高清放大走 <code>/v1/images/edits</code>；反推提示词走 <code>/v1/chat/completions</code>。高清放大默认使用 <code>gpt-image-2</code> + <code>4K</code>；节点可选择 <code>1K/2K/3K/4K</code>。gpt-image-2 请求会自动按最大 <code>3840×2160</code>、单边最长 <code>3840px</code>、宽高比最大 <code>3:1</code>、两边 <code>16px</code> 整数倍计算安全尺寸；超过 <code>2560×1440</code> 的 3K/4K 按实验性尺寸处理。多张图片会并行发送请求。
            </div>
          </div>
        </div>
      </div>
