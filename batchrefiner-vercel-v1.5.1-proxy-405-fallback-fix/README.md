...
## v1.5.1 说明
- 新增 `functions/api/proxy.js`：Cloudflare Pages 部署时也支持 `/api/proxy?url=...` 的 POST 转发，避免图片编辑 `/v1/images/edits` 在静态环境返回 HTTP 405。
- Vercel 仍使用原来的 `api/proxy.js`，Comfly API（按模型自动分组 / 4K修正版）规则不变。
