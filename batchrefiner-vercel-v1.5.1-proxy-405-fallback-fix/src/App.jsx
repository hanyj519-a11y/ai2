...
// Comfly 后台提示：gemini-3.1-flash-image-preview 默认分组没有渠道，推荐走 gemini-t3；
// nano-banana-pro / gemini-3.1-pro-preview 继续走 Gemini 优质分组。
...
const UPSCALE_DEFAULT_MODEL = 'gemini-3.1-flash-image-preview-4k';
...
function loadSettings() {
  ...
  const normalizedSavedImageModel = saved.defaultImageModel === 'nano-banana-pro-4k' ? 'nano-banana-pro' : saved.defaultImageModel;
...
function buildComflyImageBody({ model, prompt, ratio, quality, n = 1 }) {
  const imageSize = qualityToImageSize(quality);
...
function getImageModelOptions(platform, currentModel = '') {
  const baseOptions = isComflyPlatform(platform) ? COMFLY_IMAGE_MODEL_OPTIONS : CUSTOM_OPENAI_IMAGE_MODEL_OPTIONS;
  if (!isComflyPlatform(platform)) return baseOptions;
  return currentModel && !baseOptions.includes(currentModel) ? [currentModel, ...baseOptions] : baseOptions;
}
  ...
  const runSingleGeneration = async () => {
    ...
    const runImagesGeneration = async (requestModel) => {
      ...
      const body = isComflyDalleImageModel(apiModel, platform)
        ? buildComflyImageBody({ model: apiModel, prompt: singlePrompt, ratio, quality, n: 1 })
        : {
            model: apiModel,
            prompt: singlePrompt,
            n: 1,
        ...
        <div className="settings-grid">
          ...
          <div className="settings-form">
            ...
            <div className="notice-box">
              <b>怎么填：</b>已内置 <code>https://ai.comfly.chat/v1</code>，只需要给 Comfly API 填你的密钥。API 密钥只保存在你的浏览器 localStorage。当前版本分组规则：Comfly API 保持原模型列表不变，<code>gpt-image-2</code> 走 <code>default</code>；<code>gemini-3.1-flash-image-preview</code> 走 <code>gemini-t3</code>；<code>nano-banana-pro</code> 和 <code>gemini-3.1-pro-preview</code> 走 <code>Gemini优质</code>。自定义 OpenAI兼容 API 的图片生成/高清放大模型只保留 <code>gpt-image-2</code>，并新增 <code>gemini-3.1-flash-image</code>；这两个模型都按 <code>default</code> 默认分组发送，不走 gemini / Gemini优质 分组。路由规则：图片生成无参考图优先走 <code>/v1/images/generations</code>，有参考图或高清放大优先走 <code>/v1/images/edits</code>；自定义 OpenAI 兼容 API 下的 Gemini 图片模型如果上游返回 405/404，会自动退回 <code>/v1/chat/completions</code>；反推提示词走 <code>/v1/chat/completions</code>。高清放大默认使用 <code>gemini-3.1-flash-image-preview-4k</code> + <code>4K</code>，并自动走 <code>gemini-t3</code> 分组；如果接口返回的图不是 4K 长边，会在前端自动补齐到 4K 像素尺寸。多张图片会并行发送请求。
            </div>
          </div>
        </div>
      </div>
